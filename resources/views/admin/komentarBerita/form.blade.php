...
@section('content')
    ...
    <div class="border bg-gray-100 shadow-md text-gray-800 rounded-xl">
        <form class="p-5" action="{{route('komentar_berita.update', $komentarberita->id)}}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="flex rounded-md shadow-sm">
                <textarea name="isi_komen" class="flex-1 block w-full rounded-xl h-36 resize-none focus:outline-none text-lg px-3 py-1 border border-gray-300">{{$komentarberita->isi}}</textarea>
            </div>
            <label class="block text-lg font-medium text-gray-700">
                Berita
            </label>
            <div class="flex rounded-md shadow-sm">
                <select name="berita" class="flex-1 block w-full rounded-xl focus:outline-none text-lg px-3 py-1 border border-gray-300">
                    <option>Pilih Berita</option>
                    @foreach($beritas as $berita)
                        <option value="{{$berita->id}}"
                                @if($berita->id == $komentarberita->berita)
                                    selected
                                @endif
                        >{{$berita->id}}. {{$berita->judul}}</option>
                    @endforeach
                </select>
            </div>
            <div class="mt-5">
